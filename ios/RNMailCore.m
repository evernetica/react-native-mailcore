...
- (void)saveDraft:(MCOMessageBuilder *)messageBuilder folder:(NSString*)folder reject:(RCTPromiseRejectBlock)reject resolve:(RCTPromiseResolveBlock)resolve {
    MCOIMAPOperation *saveDraftOperation = [_imapSession appendMessageOperationWithFolder:@"[Gmail]/Drafts" messageData:[messageBuilder data] flags:MCOMessageFlagNone]; //flags:<#(MCOMessageFlag)#>
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    if([obj objectForKey:@"original_id"] == [NSNull null]) {
        [self saveDraft:messageBuilder folder:folder reject:reject resolve:resolve];
    }
    //  else {
    //    // this should only occur during a mail forward
    //    NSNumber *original_id = [RCTConvert NSNumber:obj[@"original_id"]];
...
- (void)parseMessagesWithContent:(NSError *)error messages:(NSArray *)messages folder:(NSString *)folder reject:(RCTPromiseRejectBlock)reject resolve:(RCTPromiseResolveBlock)resolve {
    if(error) {
        ...
        NSLog([NSString stringWithFormat:@"ids_size: %lld",ids.count]);
        [self getMailContent:ids folder:folder callback:^(NSMutableArray * emailData) {
            NSMutableDictionary *result = [[NSMutableDictionary alloc] init];
            [result setObject: @"SUCCESS" forKey: @"status"];
            [result setObject: mails forKey: @"mails"];
            NSLog([NSString stringWithFormat:@"ids_size_emailData_with_callback: %lld",emailData.count]);
            [result setObject: emailData forKey: @"mail_content"];
            resolve(result);
        }];
        // Return mails
            ...
            [operation start:^(NSError * __nullable error, NSData * messageData) {
                if (error == nil) {
                    ...
                    if (mailIds.count == emailData.count) {
                        NSLog([NSString stringWithFormat:@"ids_size_emailData: %lld",emailData.count]);
                        callback(emailData);
                    }
                } else {
